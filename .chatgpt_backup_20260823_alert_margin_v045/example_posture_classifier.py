...
class ExamplePostureClassifier:
    ...
    def classify(self, feature: NDArray[np.float32]) -> PostureClassification:
        ...
        raw_score = distance_score if projection_score is None else (0.40 * distance_score) + (0.60 * projection_score)
    ...
    def _robust_scaling(features: NDArray[np.float32]) -> tuple[NDArray[np.float32], NDArray[np.float32]]:
        ...
        scale = np.maximum(1.4826 * mad, 0.012).astype(np.float32)
        return center, scale
    ...
    def _prepare_posture_axis(self, features: NDArray[np.float32], labels: NDArray[np.int8]) -> None:
        ...
        residuals = np.concatenate([good_projection - good_scalar, bad_projection - bad_scalar])
