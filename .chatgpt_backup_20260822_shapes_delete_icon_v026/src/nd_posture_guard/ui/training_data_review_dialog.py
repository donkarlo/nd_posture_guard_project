...
class TrainingDataReviewDialog(QDialog):
    ...
    def set_samples(self, samples: list[TrainingSampleSummary]) -> None:
        """Replace the list from persistent state without changing dialog visibility."""
    ...
    def _delete_selected(self) -> None:
        """Remove the row immediately, then ask the worker to delete its persistent sample."""
        row = self._list.currentRow()
        if not (0 <= row < len(self._samples)):
            return
        sample = self._samples[row]
    ...
    def closeEvent(self, event) -> None:
        """Release preview resources before hiding/closing the dialog."""
        self._stop_video()
