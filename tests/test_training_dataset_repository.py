...
def test_new_samples_keep_video_but_skip_redundant_raw_frame_archive(tmp_path: Path) -> None:
    repo = TrainingDatasetRepository(tmp_path, novelty_multiplier=4.5, recording_fps=10.0)
    extractor = PostureGeometryFeatureExtractor()
    _sample(repo, 0, _geometry(False), extractor)
    sample = repo.list_samples()[0]
    metadata = json.loads((sample.sample_dir / "metadata.json").read_text(encoding="utf-8"))

    assert metadata["media"]["frames_archive"] is None
    assert not (sample.sample_dir / "frames.npz").exists()
    assert (sample.sample_dir / "anchor_frame.jpg").is_file()
    assert (sample.sample_dir / "features.npy").is_file()
