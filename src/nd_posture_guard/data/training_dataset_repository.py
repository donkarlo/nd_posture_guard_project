...
class TrainingDatasetRepository:
    """Persist compact review clips while building the lightweight personal geometry model."""
    ...
    def total_sample_counts(self) -> tuple[int, int]:
        """Return all persisted GOOD/BAD counts including legacy clips."""
    ...
    def save_sample(
        ...
        """Append one compact three-shape sample without storing a second raw-frame archive."""
        if label not in (0, 1):
