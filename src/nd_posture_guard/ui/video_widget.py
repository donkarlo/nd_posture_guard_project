...
class VideoWidget(QWidget):
    """Display the camera image, three-shape geometry and interactive training overlays."""
    ...
    def mousePressEvent(self, event: QMouseEvent) -> None:
        ...
        # Disable before emitting the seventh click. If validation fails,
        # MainWindow can then re-enable selection and clear stale points.
    ...
    def _draw_selected_geometry(self, painter: QPainter, draw_rect: QRect) -> None:
        """Draw the triangle/lines progressively while the user defines the three shapes."""
    ...
    def _draw_overlay(self, painter: QPainter, draw_rect: QRect) -> None:
        ...
        painter.setFont(font)
        title = self._training_shape_title() if self._training_active else "POSTURE MODEL"
        painter.drawText(
    ...
    def _training_shape_title(self) -> str:
        """Translate the seven internal landmarks into the current visible shape."""
        current = max(0, int(self._training_current))
        if current < 3:
            return f"TRAINING SHAPE 1/3 — FACE TRIANGLE {current}/3"
        if current < 5:
            return f"TRAINING SHAPE 2/3 — LEFT SHOULDER {current - 3}/2"
        if current < 7:
            return f"TRAINING SHAPE 3/3 — RIGHT SHOULDER {current - 5}/2"
        return "TRAINING SHAPES 3/3 — COMPLETE"
