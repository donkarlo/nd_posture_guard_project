...
class AutoLandmarkMainWindow(IsolatedMainWindow):
    ...
    def __init__(self, title: str, bad_score_threshold: float, data_root: str) -> None:
        ...
        for label in self.findChildren(QLabel):
            text = label.text()
            if text == "Beep threshold:":
                label.setText("Beep margin above 50%:")
                label.setToolTip(
                    "GOOD/BAD is decided at 50%. A 10% margin means audible warnings begin at 60% bad score."
                )
        ...
        self._threshold_spin.setToolTip(
            "Alert margin above the 50% GOOD/BAD boundary. "
            "Example: 10% means BAD is displayed from 50%, but the beep starts from 60%."
        )
