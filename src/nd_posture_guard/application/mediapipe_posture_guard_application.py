...
class MediaPipePostureGuardApplication:
    """Per-frame MediaPipe Tasks landmarks with no LK optical-flow propagation."""
    ...
    def __init__(self, settings_path: Path) -> None:
        ...
        model_dir = (
            Path(settings.training_data_root).expanduser()
            / "models"
            / "mediapipe"
        )
        feature_extractor = PostureGeometryFeatureExtractor()
        detector = MediaPipePostureGeometryDetector(model_dir)
