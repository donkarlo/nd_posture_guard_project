...
from nd_posture_guard.application.responsive_monitoring_worker import ResponsiveMonitoringWorker
...
class PostureGuardApplication:
    """Compose the desktop UI, camera, persistent dataset and seven-point geometry model."""
    ...
    def __init__(self, settings_path: Path) -> None:
        ...
        dataset_repository = NonBlockingTrainingDatasetRepository(
        ...
        worker = ResponsiveMonitoringWorker(
            camera=CameraReader(
                camera_device,
        ...
        self._controller = MonitoringController(
            self._window, worker, alert_service, settings_repository
        )
        self._qt_app.aboutToQuit.connect(self._controller.stop)

    def run(self) -> int:
        """Show the desktop window, start monitoring and enter the Qt event loop."""
        self._window.show()
